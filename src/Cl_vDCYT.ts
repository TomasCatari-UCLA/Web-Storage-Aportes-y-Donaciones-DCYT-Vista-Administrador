...
export default class Cl_vDCYT extends Cl_vGeneral {
  private vAporte: Cl_vAporte;

  // Vistas Principales
  private sectionDCYT: HTMLElement;
  private sectionResumen: HTMLElement;
  ...
  private lblEfectivo: HTMLElement;
  private lblEspecie: HTMLElement;
  ...
  private idAportePendiente: string | null = null;

  // Elementos Búsqueda
  private modalBuscar: HTMLElement;
  private inBusIdAporte: HTMLInputElement;
  private inBusFecha: HTMLInputElement;
  private slBusTipo: HTMLSelectElement;
  private inBusDescripcion: HTMLInputElement;
  private inBusMonto: HTMLInputElement;
  private inBusNomDonador: HTMLInputElement;
  private slBusTipoDonador: HTMLSelectElement;
  private btBuscarCancelar: HTMLButtonElement;
  private btBuscarAceptar: HTMLButtonElement;

  // Elementos Resumen Analítico
  private btCerrarResumen: HTMLButtonElement;
  private tblTipoDonacion: HTMLElement;
  private tblTipoDonador: HTMLElement;
  private tblTemporal: HTMLElement;
  private kpiUnicos: HTMLElement;
  private kpiMontoTotal: HTMLElement;
  private kpiMax: HTMLElement;
  private kpiMin: HTMLElement;
  private kpiPromedio: HTMLElement;
  private listaObservaciones: HTMLElement; // NUEVO
  ...
  constructor() {
    ...
    this.sectionDCYT = document.getElementById("dcyt") as HTMLElement;
    this.sectionResumen = document.getElementById(
      "resumenAnalitico"
    ) as HTMLElement;
    ...
    this.btCerrarResumen = document.getElementById(
      "btCerrarResumen"
    ...
    this.lblTotal = document.getElementById("lblTotalAportes") as HTMLElement;
    this.lblEfectivo = document.getElementById("lblEfectivo") as HTMLElement;
    this.lblEspecie = document.getElementById("lblEspecie") as HTMLElement;
    this.lblMontoTotal = document.getElementById(
      "lblMontoTotal"
    ) as HTMLElement;

    // Conexiones Resumen
    this.tblTipoDonacion = document.getElementById(
      "tbl_tipoDonacion"
    ) as HTMLElement;
    this.tblTipoDonador = document.getElementById(
      "tbl_tipoDonador"
    ) as HTMLElement;
    this.tblTemporal = document.getElementById("tbl_temporal") as HTMLElement;
    this.kpiUnicos = document.getElementById("kpi_unicos") as HTMLElement;
    this.kpiMontoTotal = document.getElementById(
      "kpi_montoTotal"
    ) as HTMLElement;
    this.kpiMax = document.getElementById("kpi_max") as HTMLElement;
    this.kpiMin = document.getElementById("kpi_min") as HTMLElement;
    this.kpiPromedio = document.getElementById("kpi_promedio") as HTMLElement;
    this.listaObservaciones = document.getElementById(
      "listaObservaciones"
    ) as HTMLElement; // NUEVO
    ...
    this.inBusIdAporte = document.getElementById(
      "bus_inIdAporte"
    ) as HTMLInputElement;
    this.inBusFecha = document.getElementById(
      "bus_inFecha"
    ) as HTMLInputElement;
    this.slBusTipo = document.getElementById("bus_slTipo") as HTMLSelectElement;
    this.inBusDescripcion = document.getElementById(
      "bus_inDescripcion"
    ) as HTMLInputElement;
    this.inBusMonto = document.getElementById(
      "bus_inMonto"
    ) as HTMLInputElement;
    this.inBusNomDonador = document.getElementById(
      "bus_inNomDonador"
    ) as HTMLInputElement;
    this.slBusTipoDonador = document.getElementById(
      "bus_slTipoDonador"
  ...
  mostrarAportes(listaFiltrada?: iAporte[]) {
    this.divTabla.innerHTML = "";
    let aportes = listaFiltrada ? listaFiltrada : this.controlador?.dtAportes;
    if (!aportes) return;
    ...
    if (this.lblEfectivo) this.lblEfectivo.innerHTML = efectivo.toString();
    if (this.lblEspecie) this.lblEspecie.innerHTML = especie.toString();
    ...
    aportes.forEach((aporte: iAporte, index: number) => {
      let claseColor = "txt-blue";
      if (aporte.tipo === "Efectivo") claseColor = "txt-green";
      if (aporte.tipo === "Especie") claseColor = "txt-yellow";

      html += `<div class="card">
                <div class="card-content">
                    <div class="card-title">${aporte.nombreDonador} (${
        aporte.tipoDonador
      })</div> 
                    <div class="card-detail"> <b>ID:</b> ${
                      aporte.idAporte || "N/A"
                    } | <b>Fecha:</b> ${
        new Cl_mAporte(aporte).fechaStr
      } | <b>Monto:</b> Bs. ${aporte.monto.toFixed(2)}</div>
                    <div class="card-status ${claseColor}"><span class="status-dot">●</span> ${
        aporte.tipo
      }: ${aporte.descripcion.substring(0, 40)}...</div>
                </div>
    ...
    aportes.forEach((aporte: iAporte, index: number) => {
      ...
      if (btnC) btnC.onclick = () => this.consultarAporte(aporte.idAporte);
      let btnE = document.getElementById(`dcyt_btEditar_${index}`);
      if (btnE) btnE.onclick = () => this.editAporte(aporte.idAporte);
      let btnD = document.getElementById(`dcyt_btEliminar_${index}`);
      if (btnD) btnD.onclick = () => this.pedirConfirmacion(aporte.idAporte);
    });
  }
  ...
  mostrarResumenAnalitico() {
    ...
    let todos = this.controlador?.dtAportes || [];
    ...
    let promEfec =
      efec.length > 0 ? (montoEfec / efec.length).toFixed(2) : "0.00";
    ...
    this.tblTipoDonacion.innerHTML = `
        <tr><td>Efectivo</td><td>${
          efec.length
        }</td><td>${porcEfectivo}%</td><td>Bs. ${montoEfec.toFixed(
      2
    )}</td><td>Bs. ${promEfec}</td></tr>
        <tr><td>Especie</td><td>${
          espe.length
        }</td><td>${porcEspecie}%</td><td>N/A</td><td>N/A</td></tr>
        <tr style="font-weight:bold; background:#eee;"><td>TOTAL</td><td>${total}</td><td>100%</td><td>Bs. ${montoEfec.toFixed(
      2
    )} (Cash)</td><td>-</td></tr>
      `;
    ...
    let cashJur = juridica
      ...
      .reduce((s, c) => s + c.monto, 0);
    let promNat =
      natural.filter((a) => a.tipo === "Efectivo").length > 0
        ? (
            cashNat / natural.filter((a) => a.tipo === "Efectivo").length
          ).toFixed(2)
        : "0.00";
    let promJur =
      juridica.filter((a) => a.tipo === "Efectivo").length > 0
        ? (
            cashJur / juridica.filter((a) => a.tipo === "Efectivo").length
          ).toFixed(2)
        : "0.00";

    this.tblTipoDonador.innerHTML = `
        <tr><td>Naturales</td><td>${natural.length}</td><td>${(
      (natural.length / total) *
      100
    ).toFixed(1)}%</td><td>Bs. ${cashNat.toFixed(
      2
    )}</td><td>Bs. ${promNat}</td></tr>
        <tr><td>Jurídicas</td><td>${juridica.length}</td><td>${(
      (juridica.length / total) *
      100
    ).toFixed(1)}%</td><td>Bs. ${cashJur.toFixed(
      2
    )}</td><td>Bs. ${promJur}</td></tr>
      `;
    ...
    todos.forEach((a) => {
      ...
      if (fStr.length === 8) {
        ...
        if (mes >= 1 && mes <= 3) q1++;
        else if (mes >= 4 && mes <= 6) q2++;
        else if (mes >= 7 && mes <= 9) q3++;
        else if (mes >= 10 && mes <= 12) q4++;
      }
    });
    ...
    let nombreQ = "";
    if (maxQ === q1) nombreQ = "Primer trimestre (Ene-Mar)";
    else if (maxQ === q2) nombreQ = "Segundo trimestre (Abr-Jun)";
    else if (maxQ === q3) nombreQ = "Tercer trimestre (Jul-Sep)";
    else nombreQ = "Último trimestre (Oct-Dic)";
  ...
  ejecutarBusqueda() {
    ...
    let filtrados = todos.filter((e: iAporte) => {
      let coincide = true;
      let eModel = new Cl_mAporte(e);
      if (sIdAporte && !e.idAporte.toLowerCase().includes(sIdAporte))
        coincide = false;
      if (sFecha && !eModel.fecha.toString().includes(sFecha)) coincide = false;
      if (sTipo && e.tipo !== sTipo) coincide = false;
      if (sDescripcion && !e.descripcion.toLowerCase().includes(sDescripcion))
        coincide = false;
      if (sMonto && String(e.monto) !== sMonto) coincide = false;
      if (sNomDonador && !e.nombreDonador.toLowerCase().includes(sNomDonador))
        coincide = false;
      if (sTipoDonador && e.tipoDonador !== sTipoDonador) coincide = false;
      return coincide;
    });
  ...
  consultarAporte(idAporte: string) {
    let aporte = this.controlador?.aporte(idAporte);
    if (aporte)
      this.controlador?.activarVista({
        vista: "aporte",
  ...
  pedirConfirmacion(idAporte: string) {
